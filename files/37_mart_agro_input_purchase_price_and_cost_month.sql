-- 37_mart_agro_input_purchase_price_and_cost_month.sql
-- Year-agnostic пересборка денежной модели агрохимии на едином raw.r1c_purchase_lines
-- (2021-2026, загружается автоматически через dag_extract_purchases).
-- Логика цены скопирована 1:1 из mart.v_agro_input_purchase_price_2026, только
-- источник заменён с raw.r1c_purchase_lines_2026 на raw.r1c_purchase_lines
-- + join с raw.r1c_purchase_headers за организацией/контрагентом/НДС-флагом.
...
WHERE n."АпкНазначение" = ANY (ARRAY['СЗР', 'Удобрение']);

GRANT SELECT ON mart.v_agro_input_purchase_price TO datalens_ro;

-- ============================================================================
-- Денежная помесячная витрина: натуральный факт применения (списание на поле)
-- + последняя подтверждённая закупочная цена на дату списания или раньше.
-- Если цены на дату списания или раньше нет — берём ближайшую БУДУЩУЮ цену
-- и помечаем как fallback (как было в предыдущей 2026-модели).
-- ============================================================================
...
by_field_month AS (
    SELECT
        ...
        c.field_sk,
        n."Description"                           AS category_placeholder,
        CASE

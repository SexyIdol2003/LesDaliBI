...
## Известные ограничения источников данных

### fact_putevoy_rabota.pole_id всегда NULL

Расследовано 15.09.2026 (см. issue #2 в репозитории). Путевой лист тракториста
(`Document_АпкПутевойЛистТракториста`) в этой конфигурации 1С не хранит привязку
к полю на уровне строки работы:

- `КлючСвязи` в табличных частях `ВыполненныеРаботы`/`ПереченьОборудования` —
  внутренняя связь строк одного документа (работа <-> единица техники в агрегате),
  не ссылка на другой документ.
- Табличная часть `Задание` (единственное потенциальное место для `Поле_Key`)
  пуста абсолютно по всей базе 1С — механизаторы её не заполняют.

**Вывод:** это ограничение бизнес-процесса заказчика, не баг ETL. Детализацию
полеводства по полю нужно строить на других источниках, где `Поле_Key` реально
есть: `raw.r1c_dvizhenie_produkcii_lines`, `raw.r1c_tok_vzveshivanie_talony`,
`raw.r1c_polya_istoriya`. Диагностический скрипт: `27_diagnose_putevoy_pole_id_linkage.sql`.

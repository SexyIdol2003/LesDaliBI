...
def main() -> int:
    if len(sys.argv) != 2:
        print("Использование: python3 export_purchase_lines_history.py <year>", file=sys.stderr)
        return 1
    ...
    headers_path = Path(f"purchase_headers_{year}.jsonl")
    out_path = Path(f"purchase_lines_{year}.jsonl")
    progress_path = Path(f"purchase_lines_{year}.progress")

    if not headers_path.exists():
        print(f"Не найден файл шапок: {headers_path}", file=sys.stderr)
        return 1
    ...
    print(f"Год {year}: кандидатов на детальную выгрузку: {len(candidates)}")

...
entity = "Document_ДвижениеПродукцииИМатериалов"
page_size = 100
target_year = 2026
target_org = "cc598f91-f31b-40a6-ae6e-b1eb63a25dee"
target_doc_type = "АктНаСписанияСемянУдобренийИЯдов"

out_path = Path("movement_headers_2026_agro_writeoff.jsonl")

params_base = {
    "$format": "json",
    "$select": (
        "Ref_Key,Date,Number,Posted,Организация_Key,"
        "ХозяйственнаяОперация,АпкВидДокумента,"
        "АпкВидРаботы_Key,АпкОбъектЗатрат_Key"
    ),
}
...
with out_path.open("w", encoding="utf-8") as out:
    while True:
        ...
        for row in rows:
            ...
            if (
                row.get("Posted") is True
                and row.get("Организация_Key") == target_org
                and row.get("АпкВидДокумента") == target_doc_type
            ):
        ...
        print(
            ...
            f"| просмотрено={total_seen:>5} "
            f"| всего {target_year}={year_2026:>4} "
            f"| актов списания={selected:>4}",
            flush=True,
        )
...
print(f"Документов за {target_year}: {year_2026}")
print(f"Проведённых актов списания семян/удобрений/ядов: {selected}")
